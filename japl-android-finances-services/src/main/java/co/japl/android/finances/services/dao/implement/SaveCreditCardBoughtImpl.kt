...
class SaveCreditCardBoughtImpl @Inject constructor(
    val context: Context,
    override var dbConnect: SQLiteOpenHelper
) :
    IQuoteCreditCardDAO {
    ...
    override fun delete(id: Int): Boolean {
        ...
        differInstallmentSvc.get(id)
            ...
            ?.let { differ ->
                val differ = get(id)
                getDifferOriginBought(
                    differ.get().nameItem,
                    differ.get().boughtDate,
                    differ.get().endDate
                )
                    ?.let { bought ->
        ...
        return (db.delete(
            CreditCardBoughtDB.CreditCardBoughtEntry.TABLE_NAME,
            DatabaseConstants.SQL_DELETE_CALC_ID, arrayOf(id.toString())
        ) > 0).also { Log.v(this.javaClass.name, "<<<=== delete - End $it") }
    }

    @RequiresApi(Build.VERSION_CODES.O)
    fun getDifferOriginBought(
        name: String,
        ...
        db.query(
            ...
            """
             ...
             """,
            arrayOf(
                DateUtils.toSeconds(boughtDate.withHour(0).withMinute(0).withSecond(0)).toString(),
                DateUtils.toSeconds(boughtDate.withHour(11).withMinute(59).withSecond(59))
                    .toString(),
                name,
                ...
                with(cursor) {
                    if (moveToNext()) {
                        return CreditCardBoughtMap().mapping(this)
                            .also { Log.d(javaClass.name, "<<<=== END:getDifferOriginBought $it") }
                    }
                }
            }
        return null
    ...
    override fun getToDate(
        ...
        try {
            Log.d(
                this.javaClass.name,
                "<<<=== STARTING::getToDate $startDate ..  $cutOffDate CC: $key"
            )
            val db = dbConnect.readableDatabase
            db.query(
                CreditCardBoughtDB.CreditCardBoughtEntry.TABLE_NAME,
    ...
    override fun getPendingQuotes(
        ...
        Log.d(
            this.javaClass.name,
            "<<<=== START::getPendingQuotes Key: $key Start: $startDate Cutoff: $cutoffCurrent"
        )
        val db = dbConnect.readableDatabase
        val items = mutableListOf<CreditCardBoughtDTO>()
        db.query(
            CreditCardBoughtDB.CreditCardBoughtEntry.TABLE_NAME, COLUMNS_CALC,
            """
        ...
        return items.also {
            Log.d(
                this.javaClass.name,
                "<<<=== FINISH::getPendingQuotes Size: ${it.size}"
            )
        }
    }
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    private fun getValidMonths(
        creditCardBoughtDTO: CreditCardBoughtDTO,
        cutoffCurrent: LocalDateTime,
        startCutOff: LocalDateTime
    ): Boolean {
        ...
        return (differInstallment.takeIf { it.isPresent }?.let {
            return (DateUtils.getMonths(
                LocalDateTime.of(it.get().create, LocalTime.MAX),
                cutoffCurrent
            ) < it.get().newInstallment).also {
            ...
            creditCardBoughtDTO.boughtDate,
            cutoffCurrent
        ) < creditCardBoughtDTO.month)
                ).also {
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    override fun getBoughtPendingQuotes(
        key: Int,
        startDate: LocalDateTime,
        cutOff: LocalDateTime
    ): Optional<Long> {
        ...
        val value = list.stream().filter {
            differQuotes.firstOrNull { differ -> differ.cdBoughtCreditCard.toInt() == it.id }?.let {
                DateUtils.getMonths(
                    LocalDateTime.of(it.create, LocalTime.MAX),
                    cutOff
                ) <= it.newInstallment
            } ?: (DateUtils.getMonths(it.boughtDate, cutOff) <= it.month)
        }.count()
        return Optional.ofNullable(value)
            .also { Log.d(this.javaClass.name, "<<<=== getBoughtPendingQuotes - End $it") }
    }

    @RequiresApi(Build.VERSION_CODES.TIRAMISU)
    override fun getBoughtQuotes(
        key: Int,
        startDate: LocalDateTime,
        cutOff: LocalDateTime
    ): Optional<Long> {
        ...
        val countRecurrent = listRecurrent.count { it.month > 1 }
        val value = list.count { it.month > 1 }
        return Optional.ofNullable((value + countRecurrent).toLong())
            .also { Log.d(this.javaClass.name, "<<<=== getBoughtQuotes - End $it") }
    }

    @RequiresApi(Build.VERSION_CODES.TIRAMISU)
    override fun getPendingToPay(
        key: Int,
        startDate: LocalDateTime,
        cutOff: LocalDateTime
    ): Optional<BigDecimal> {
        ...
        val value = list.stream().map {
            val differ =
                differQuotes.firstOrNull { differ -> differ.cdBoughtCreditCard.toInt() == it.id }
            val months = differ?.let { DateUtils.getMonths(it.create, cutOff) }
                ?: DateUtils.getMonths(it.boughtDate, cutOff)
            val quote = differ?.let { (it.pendingValuePayable / it.newInstallment).toBigDecimal() }
                ?: it.valueItem.divide(it.month.toBigDecimal(), 8, RoundingMode.CEILING)
            val bought = quote.multiply(months.toBigDecimal())
            val capital = differ?.let { it.pendingValuePayable.toBigDecimal().minus(bought) }
    ...
    @RequiresApi(Build.VERSION_CODES.O)
    override fun getPendingToPayQuotes(
        key: Int,
        startDate: LocalDateTime,
        cutOff: LocalDateTime
    ): Optional<BigDecimal> {
    ...
    override fun getRecurrentPendingQuotes(
        ...
        while (status) {
            items.forEach {
                val copy = it.copy()
                copy.boughtDate =
                    it.boughtDate.withMonth(cutOffNew.monthValue).withYear(cutOffNew.year)
                val startCutOff = DateUtils.startDateFromCutoff(cutOffDay, cutOffNew)
                if (DateUtils.getMonths(copy.boughtDate, cutOff) < it.month && copy.endDate.isAfter(
                        startCutOff
                    )
                ) {
                    itemsTemp.add(copy)
        ...
        return itemsTemp.also {
            Log.d(
                javaClass.name,
                "<<<=== ENDING::getRecurrentPendingQuotes Size: ${it.size}"
            )
        }
    }
    ...
    override fun endingRecurrentPayment(idBought: Int, cutOff: LocalDateTime): Boolean {
        Log.d(
            javaClass.name,
            "<<<=== STARTING::endingRecurrentPayment Id: $idBought CutOff: $cutOff"
        )
        ...
        if (bought.isPresent) {
            ...
            if (creditCard.isPresent) {
                val endDate =
                    DateUtils.startDateFromCutoff(creditCard.get().cutOffDay, cutOff).minusDays(2)
                bought.get().endDate = endDate
                return (save(bought.get()) > 0).also {
                    Log.d(
                        javaClass.name,
                        "<<<=== ENDING::endingRecurrentPayment Id: $idBought cutOff: $cutOff EndDate: $endDate Response: $it"
                    )
                }
            }
        }
        return false.also {
            Log.d(
                javaClass.name,
                "<<<=== ENDING::endingRecurrentPayment BAD Validation Id: $idBought cutOff: $cutOff Response: $it"
            )
        }
    }
    ...
    override fun endingPayment(idBought: Int, message: String, cutOff: LocalDateTime): Boolean {
        ...
        if (bought.isPresent) {
            ...
            if (creditCard.isPresent) {
                ...
                return (save(bought.get()) > 0).also {
                    Log.d(
                        javaClass.name,
                        "<<<=== ENDING::endingPayment Id: $idBought cutOff: $cutOff EndDate: $endDate Response: $it"
                    )
                }
            }
        }
        return false.also {
            Log.d(
                javaClass.name,
                "<<<=== ENDING::endingPayment BAD Validation Id: $idBought cutOff: $cutOff Response: $it"
            )
        }
    }
    ...
    override fun restoreBackup(pathFile: String) {
        ...
        val list = Files.newBufferedReader(path, Charset.defaultCharset())
            .use { Gson().fromJson(it, List::class.java) } as List<CreditCardBoughtDTO>
    ...
    override fun getLastAvailableQuotesTC(): List<QuoteCreditCard> {
        ...
        for (creditCard in creditCards) {
            ...
            dto.capitalValue =
                Optional.of(capital.orElse(BigDecimal.ZERO) + capitalQuotes.orElse(BigDecimal.ZERO))
            dto.interestValue =
                Optional.of(interestQuote.orElse(BigDecimal.ZERO) + interest.orElse(BigDecimal.ZERO))
            list.add(dto)
        }
        return list
    ...
    override fun getPeriod(creditCardId: Int): List<YearMonth> {
        ...
        return items.distinct()
            .also { Log.i(this.javaClass.name, "<<<=== FINISH Get all records Count ${it.size}") }
    }
            ...
            while (cursor.moveToNext()) {
                CreditCardBoughtMap().mapping(cursor).takeIf {
                    it.boughtDate.toLocalDate().compareTo(boughtDate.toLocalDate()) == 0 ||
                            it.boughtDate.toLocalDate() in boughtDate.toLocalDate()..boughtDate.plusDays(
                        5
                    ).toLocalDate()
                }?.let(list::add)
            }
            ...
            val cursor = db.rawQuery(
                ...
                with(cursor) {
                    while (moveToNext()) {
                        ...
                        val boughtDate = CursorUtils.getLocalDateTime(
                            cursor,
                            CreditCardBoughtDB.CreditCardBoughtEntry.COLUMN_BOUGHT_DATE
                        )
                        db.execSQL(
            ...
            val cursor = db.rawQuery(
                ...
                with(cursor) {
                    while (moveToNext()) {
                        ...
                        val id = CursorUtils.getInt(cursor, BaseColumns._ID)
                        val endDate = CursorUtils.getLocalDateTime(
                            cursor,
                            CreditCardBoughtDB.CreditCardBoughtEntry.COLUMN_END_DATE
                        )
