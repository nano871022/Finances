...
import co.com.japl.module.paid.views.periods.list.fakes.FakePeriodPaidPort
...
private fun Body(viewModel: PeriodsViewModel, navController: NavController) {
    ...
    Column(modifier = Modifier.padding(Dimensions.PADDING_SHORT)) {
        Header(
            count = listState.values.flatMap { it }.sumOf { it.count },
...
private fun Item(item: PeriodPaidDTO, goTo: (YearMonth) -> Unit) {
    val optionsState = remember { mutableStateOf(false) }
    ...
    if (optionsState.value) {
        MoreOptionsDialog(
            listOptions = PeriodListOptions.values().toList(),
...
internal fun PeriodPreview() {
    val paidSvc: IPeriodPaidPort = FakePeriodPaidPort()
    MaterialThemeComposeUI {
        Period(
            viewModel(
                factory = PeriodsViewModel.Companion.create(
                    extras = viewModel(),
                    paidSvc = paidSvc
                )
            ),
