...
private fun Body(viewModel: PaidViewModel, navController: NavController) {
    Scaffold(floatingActionButton = {
        ...
        Column {
            Header(viewModel)
            List(viewModel, navController, modifier = Modifier.padding(it))
        }
    }
}
...
private fun Buttons(newOne: () -> Unit) {
    Column {
        FloatButton(
            ...
            descriptionIcon = R.string.add_paid,
            onClick = newOne
        )
    }
}
...
@Composable
private fun Header(viewModel: PaidViewModel) {
    ...
    Row {
        ...
        FieldView(
            name = stringResource(id = R.string.value),
            value = NumbersUtil.toString(viewModel.allValues.value),
            modifier = Weight1f()
        )
        IconButton(
...
@Composable
private fun Settings(settingState: MutableState<Boolean>, prefs: Prefs?) {
    val daysSmsReadState = remember { mutableStateOf(prefs?.paidSMSDaysRead?.toString() ?: "0") }
    Popup(title = R.string.setting, state = settingState) {
        Scaffold(floatingActionButton = {
            ...
            Column(
                ...
                FieldText(
                    title = stringResource(id = R.string.msm_read_num),
...
@Composable
private fun List(viewModel: PaidViewModel, navController: NavController, modifier: Modifier) {
...
@Composable
private fun Items(list: List<PaidDTO>?, viewModel: PaidViewModel, navController: NavController) {
    val visibleContentState = remember { mutableStateOf(true) }
    list?.takeIf { it.isNotEmpty() }?.let {
        Surface(
            ...
            Column(modifier = Modifier.padding(Dimensions.PADDING_SHORT)) {
                HeaderMonthly(
                    period = YearMonth.of(
                        list[0].datePaid.year,
                        list[0].datePaid.monthValue
                    ),
                    value = list.sumOf { it.itemValue },
                    onClick = { visibleContentState.value = !visibleContentState.value })

                if (visibleContentState.value) {
                    LazyColumn {
                        items(list.size) { index ->
                            Item(dto = list[index], viewModel = viewModel, navController = navController)
                        }
                    }
                }
            }
...
@Composable
private fun HeaderMonthly(period: YearMonth, value: Double, onClick: () -> Unit) {
    Row(
...
@Composable
private fun Item(dto: PaidDTO, viewModel: PaidViewModel, navController: NavController) {
...
internal fun PaidPreview() {
    ...
    MaterialThemeComposeUI {
        Paid(
            viewModel(
                factory = PaidViewModel.Companion.create(
                    extras = viewModel(),
                    paidSvc = paidSvc,
                    prefs = prefs
                )
