...
@Composable
internal fun RecordBoughtCreditCard(bought: CreditCardBoughtItemDTO,
                                    creditCard: CreditCardDTO,
                                    differQuotes:List<DifferInstallmentDTO>,
                                    cutOff: LocalDateTime,
                                    view:View= LocalView.current,
                                    navController: NavController? = Navigation.findNavController(view),
                                    prefs:Prefs,
                                    loader:MutableState<Boolean>,
                                    colorPendingValue:Color = Color.Unspecified,
                                    boughtCCSvc: IBoughtListPort,
                                    simulatorSvc: ISimulatorCreditVariablePort) {
    val context = LocalContext.current
    val application = context.applicationContext

    val state = remember { mutableStateOf(false) }
    val model = remember {
        BoughtViewModel (application = application as Application,prefs, loader = loader,boughtCCSvc,simulatorSvc)
            .setBought(bought)
