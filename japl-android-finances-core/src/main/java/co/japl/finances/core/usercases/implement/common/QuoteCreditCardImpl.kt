...
,private val context:Context): IQuoteCreditCard {
    ...
    private fun calculateInterest(creditCard:CreditCardDTO?,dto:CreditCardBoughtDTO,tax: Pair<Double, String>?,taxCashAdv: Pair<Double, String>?,taxWalletBuy: Pair<Double, String>?,cutOff: LocalDateTime):BigDecimal{
        ...
        val interest = KindOfTaxImpl.getNM(
            tax?.first ?: defaultTax.first,
            KindOfTaxEnum.findByValue(tax?.second ?: defaultTax.second)
                ?: KindOfTaxEnum.MONTHLY_EFFECTIVE
        )
        val interestCashAdv = KindOfTaxImpl.getNM(
            taxCashAdv?.first ?: defaultTax.first,
            KindOfTaxEnum.findByValue(taxCashAdv?.second ?: defaultTax.second)
                ?: KindOfTaxEnum.MONTHLY_EFFECTIVE
        )
        val interestWalletBuy = KindOfTaxImpl.getNM(
            taxWalletBuy?.first ?: defaultTax.first,
            KindOfTaxEnum.findByValue(taxCashAdv?.second ?: defaultTax.second)
                ?: KindOfTaxEnum.MONTHLY_EFFECTIVE
        )
        var setting = false
        ...
        buyCCSettingSvc.get(dto.id)?.let {
            ...
            creditCardSettingSvc.get(it.id)?.let{
